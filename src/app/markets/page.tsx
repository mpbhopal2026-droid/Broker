...
export default function MonochromeMarketsPage() {
  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto select-none">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pt-1">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all shrink-0 ${
                selectedCategory === cat.id
                  ...
                  : 'bg-white dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-800 hover:bg-[#e6f4ea] hover:text-[#00875a]'
              }`}
            >
              {cat.label}
            </button>
        ...
        <div className="relative w-full sm:w-60">
          ...
          <input
            ...
            className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg pl-8 pr-3 py-1.5 text-xs text-zinc-950 dark:text-white placeholder-zinc-400 focus:outline-none focus:border-[#00875a]"
          />
        </div>
      ...
      {/* Markets Asset Table */}
      <div className="rounded-xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-2xs">
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      ...
                      <span className={`text-[10px] font-semibold tabular-nums ${isUp ? 'text-[#00875a]' : 'text-rose-600'}`}>
                        {isUp ? '▲ +' : '▼ -'}{Math.abs(asset.changePercent).toFixed(2)}%
                      </span>
                    </div>
                    ...
                    <a
                      ...
                      rel="noopener noreferrer"
                      onClick={(e) => e.stopPropagation()}
                      className="p-1.5 rounded-md border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 text-zinc-600 hover:text-[#00875a] transition-colors"
        ...
        <div className="hidden sm:block overflow-x-auto">
          <table className="w-full text-left text-xs font-sans">
            <thead className="bg-zinc-50/70 dark:bg-zinc-900/50 text-zinc-400 border-b border-zinc-100 dark:border-zinc-900 text-[10px] uppercase font-bold">
              <tr>
                <th className="py-3 px-3 w-8"></th>
                <th className="py-3 px-3">Instrument</th>
                <th className="py-3 px-3">Bid</th>
                <th className="py-3 px-3">Ask</th>
                <th className="py-3 px-3">24h Delta</th>
                ...
                filteredAssets.map((asset) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Bid */}
                      <td className="py-3 px-3 font-bold tabular-nums text-zinc-950 dark:text-white">
                        ${(asset.bid ?? asset.price).toFixed(decimals)}
                      </td>

                      {/* Ask */}
                      <td className="py-3 px-3 text-zinc-600 dark:text-zinc-400 tabular-nums font-medium">
                        ${(asset.ask ?? asset.price).toFixed(decimals)}
                      </td>

                      {/* Change */}
                      <td className="py-3 px-3">
                        <span className={`font-bold tabular-nums flex items-center gap-1 ${isUp ? 'text-[#00875a]' : 'text-rose-600'}`}>
                          <span>{isUp ? '▲' : '▼'}</span>
                          <span>{isUp ? '+' : ''}{Math.abs(asset.changePercent).toFixed(2)}%</span>
                        </span>
                      </td>

                      {/* Actions: [ Trade ] and [ Chart ↗ ] */}
