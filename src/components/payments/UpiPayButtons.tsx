...
export const UpiPayButtons: React.FC<{ params: UpiLinkParams | null }> = ({ params }) => {
  ...
  if (!links || !params) return null;

  const handleCopyUpi = () => {
    if (navigator.clipboard) {
      navigator.clipboard.writeText(params.vpa);
      setCopied(true);
      setTimeout(() => setCopied(false), 2500);
    }
  };

  const handleUpiClick = (e: React.MouseEvent<HTMLAnchorElement>, targetHref: string) => {
    if (!isMobile) {
      e.preventDefault();
      handleCopyUpi();
    }
  ...
  return (
    <div className="space-y-2">
      <a
        href={links.generic}
        onClick={(e) => handleUpiClick(e, links.generic)}
        className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-slate-950 dark:bg-white text-white dark:text-slate-950 font-bold text-xs active:scale-95 transition-transform cursor-pointer shadow-2xs"
      >
        <Smartphone className="w-4 h-4" aria-hidden="true" />
        {isMobile ? 'Pay with any UPI app' : copied ? '✓ UPI ID Copied to Clipboard!' : 'Copy UPI ID & Pay in App'}
      </a>
      ...
      <p className="text-[10px] text-slate-500 dark:text-slate-400 text-center leading-relaxed">
        {isMobile 
          ? 'After completing payment in your UPI app, submit the 12-digit UTR/Ref number below.'
          : 'Scan the QR code above with your mobile banking or UPI app, then submit the 12-digit UTR below.'}
      </p>
    </div>
  );
};
