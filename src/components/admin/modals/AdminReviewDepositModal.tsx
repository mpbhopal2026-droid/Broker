...
export const AdminReviewDepositModal: React.FC<AdminReviewDepositModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#0d121c] border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-scale-in max-h-[92vh] flex flex-col">
        
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white">Review Deposit #{deposit.id.slice(0, 8)}</h3>
            <p className="text-xs text-slate-500">Client: {deposit.userFullName || deposit.userId}</p>
          </div>
